...
public class ActionRemoveOnEscape extends AbstractPComponentAction implements PComponentAction {
	...
	public boolean isEnabled(PRoot root) {
		...
		PMenuBody body = focusOwner.getAncestorsAndSelf().getNextOfType(PMenuBody.class);
		return body != null;
	}
	
	@Override
	public void tryToPerform(PRoot root) {
		PMenuBody body = root.getFocusOwner().getAncestorsAndSelf().getNextOfType(PMenuBody.class);
		if (body != null) {

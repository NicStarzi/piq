...
public class SwingPTest_InnerPRoot extends AbstractSwingPTest {
	...
	protected void buildGUI() {
		...
		PButton innerBtn = new PButton();
		innerBody.addChild(innerBtn, PBorderLayout.Constraint.BOTTOM);
		
		root.setBody(body);
		
		PTimer innerRootTimer = new PTimer(root, () -> innerRoot.getDummyRoot().update(1));
		innerRootTimer.setOwner(root);
		innerRootTimer.setRepeating(true);
		innerRootTimer.setDelay(1);
		root.registerTimer(innerRootTimer);
		innerRootTimer.start();
	}
